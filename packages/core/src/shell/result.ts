...
export function notice(outcome: Pick<Outcome, "status" | "exit">) {
  ...
  if (outcome.status === "timeout") return "Timed out before completion"
  if (outcome.exit === 0) return undefined
  return `Exited with code ${outcome.exit ?? "unknown"}`
}

/** Model-visible text: the bounded output followed by how the command ended, each part only when present. */
export function text(outcome: Outcome) {
  if (outcome.status === "unavailable") return missing
  return [outcome.output, notice(outcome)].filter((part) => part !== undefined && part !== "").join("\n\n")
}
